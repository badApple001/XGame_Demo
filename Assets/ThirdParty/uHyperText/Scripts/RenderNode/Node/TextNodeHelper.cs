...
namespace WXB
{
    public partial class TextNode : NodeBase
    {
        struct TextNodeHelper
        {
            ...
            public TextNodeHelper(float maxWidth, RenderCache cache, float x, uint yline, List<Line> lines, Anchor xFormatting, float offsetX, float offsetY, StringBuilder sb)
            {
            ...
            public void Draw(TextNode n, float lineX)
            {
                ...
                int textindex = 0;
                float newx = 0f;
                for (int k = 0; k < node.m_Elements.Count; ++k)
                {
                    Element e = node.m_Elements[k];
