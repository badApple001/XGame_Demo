...
namespace WXB
{
    public static class DrawFactory
    {
        ...
        public static class Pool<T> where T : MonoBehaviour, IDraw, new ()
        {
        ...
        public class Factory<T> : IFactory where T : MonoBehaviour, IDraw, new()
        {
            public IDraw Create(GameObject go)
            {
                return Pool<T>.GetOrCreate(go);
            }

            public void Free(IDraw d)
            {
                Pool<T>.Free((T)d);
            }
        }
        ...
        static public IDraw Create(GameObject go, DrawType type)
        {
            IFactory f = Get(type);
            if (f == null)
                return null;
        ...
        static public void Free(IDraw go)
        {
            if (go == null)
                return;
