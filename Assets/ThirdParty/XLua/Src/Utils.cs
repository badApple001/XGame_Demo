			...
			int obj_field, int obj_getter, int obj_setter, int obj_meta, out LuaCSFunction item_getter, out LuaCSFunction item_setter, BindingFlags access)
		{
			...
			for (int i = 0; i < methods.Length; ++i)
			{
				...
				else if (method_name.StartsWith("get_") && method.IsSpecialName && method.GetParameters().Length != 1) // getter of property
				{
					...
					MethodWrapsCache.s_ListTemp.Clear();
					MethodWrapsCache.s_ListTemp.Add(method);

					translator.PushFixCSFunction(L, translator.methodWrapsCache._GenMethodWrap(method.DeclaringType, prop_name, MethodWrapsCache.s_ListTemp).Call);
			...
			try
			{
				...
				switch (memberType)
				{
					...
					case LazyMemberTypes.PropertySet:
						...
						MethodWrapsCache.s_ListTemp.Clear();
						MethodWrapsCache.s_ListTemp.Add((memberType == LazyMemberTypes.PropertyGet) ? prop.GetGetMethod() : prop.GetSetMethod());

						wrap = translator.methodWrapsCache._GenMethodWrap(prop.DeclaringType, prop.Name, MethodWrapsCache.s_ListTemp).Call;
						break;
