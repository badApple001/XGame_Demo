...
namespace XLua
{
    ...
    {
        //临时列表变量
        public static List<MethodBase> s_ListTemp = new List<MethodBase>();
        ...
        public LuaCSFunction GetDelegateWrap(Type type)
        {
            ...
            if (!delegateCache.ContainsKey(type))
            {
                ...
                MethodWrapsCache.s_ListTemp.Add(type.GetMethod("Invoke"));
                //delegateCache[type] = _GenMethodWrap(type, type.ToString(), new MethodBase[] { type.GetMethod("Invoke") }).Call;
                delegateCache[type] = _GenMethodWrap(type, type.ToString(), MethodWrapsCache.s_ListTemp).Call;
            }
