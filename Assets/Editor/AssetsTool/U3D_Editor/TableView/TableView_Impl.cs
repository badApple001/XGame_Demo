    ...
    public partial class TableView<T>
    {
        private void DrawTitle(float width)
        {
            for (int i = 0; i < m_descArray.Count; i++)
            {
                ...
                GUI.Label(r, desc.TitleText + (selected ? _appearance.GetSortMark(_descending) : ""),
                    _appearance.GetTitleStyle(selected));
        ...
        private void DrawLine(int pos, T obj, float width)
        {
            ...
            if (m_selected!=null&&m_selected.Equals(obj))
            {
        ...
        private void DrawLineCol(int pos, int col, float width, T obj, GUIStyle style,
            bool selectionHappens = false)
        {
            ...
            // note that the 'selected-style' assignment below should be isolated from the if-conditional statement above
            // since the above if is a one-time event, on the contrary, the 'selected-style' assignment below should be done every time in the drawing process
            if (m_selectedCol == col && m_selected!=null&&m_selected.Equals(obj))
            {
        ...
        private void SortData()
        {
            m_lines.Sort((s1, s2) =>
            {
	            ...
	            var rt = m_descArray[_sortSlot].Compare(s1, s2) * (_descending ? -1 : 1);
	            if(rt==0)
	                for (int i = 0; i < m_descArray.Count; i++)
	                {
	                    if (i != _sortSlot)
	                        rt = m_descArray[i].Compare(s1, s2) * (_descending ? -1 : 1);
	                    if (rt != 0)
	                        return rt;
	                }
	            return rt;
        ...
        private Rect LabelRect(float width, int slot, int pos)
        {
            ...
            return new Rect(width * accumPercent, pos * _appearance.LineHeight,
                width * m_descArray[slot].WidthInPercent, _appearance.LineHeight);
        }
        ...
        List<T> m_lines = new List<T>();

        T m_selected =default(T);
        int m_selectedCol = -1;

        Dictionary<T, Color> m_specialTextColors;
    }
}
