...
{
    public class SpineCreatureTargetMoverPart : BasePart
    {
        ...
        {
            m_stop = true;
            m_actor?.GetSkeleton().state.SetAnimation(0, m_actor?.GetAnimConfig().szIdle, true);
        }
        public SpineCreatureTargetMoverPart SetPath(List<Vector3> path)
        {
            // m_moveType = EMoveType.StepPath;
            m_path = path;
            return this;
        }
        public SpineCreatureTargetMoverPart SetDestination(Vector3 destination)
        {
        ...
        public SpineCreatureTargetMoverPart SetArriveCallback(Action callback)
        {
